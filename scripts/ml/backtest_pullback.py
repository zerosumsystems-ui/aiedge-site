#!/usr/bin/env python3
"""Backtest of the small-pullback breakout-stop setup, traded by SCALING
IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to a third setup. The
simulate logic is a deliberate per-setup copy — each backtest stays
self-contained and independently tunable.

The detector (scripts/live/pullback_detector.py) is unchanged: a strong
impulse leg, a brief shallow pullback that holds the EMA, then a
with-trend prior-bar breakout-stop entry. Only the TRADE MANAGEMENT is
the scaling-in method.
...
For the pullback the measured move is the impulse leg projected from
the breakout-stop entry; the wide stop is widened beyond the pullback's
far side. Longs and shorts use identical mirrored rules — no bias.
...
downloaded 5-minute RTH bars; a bar straddling stop and target is scored
stopped, and a bar reaching the wide stop is assumed to fill every
pending add first (both conservative).
...
from pullback_detector import Bar, PullbackConfig, detect_pullbacks  # noqa: E402
...
SCALE_STEP_FRAC = 0.45       # tranche spacing, as a fraction of the
                             # impulse-leg height
STOP_WIDEN_FRAC = 2.50       # the wide stop sits this fraction of the
                             # impulse height beyond the structural stop
BREAKEVEN_ARM_FRAC = 0.15    # once favourable excursion clears the
...
def load_sessions() -> list[tuple[str, str, list[Bar]]]:
    """Read every downloaded 5-minute RTH session under public/analogs/."""
    out: list[tuple[str, str, list[Bar]]] = []
    for d in sorted(p for p in ANALOGS_DIR.iterdir() if p.is_dir()):
        ...
        for i in range(len(c)):
            ...
            bars.append(Bar(t=t, o=o[i] / scale, h=h[i] / scale,
...
def setups_for_session(bars: list[Bar]) -> list[TradeSetup]:
    """Turn this session's pullback signals into normalised TradeSetups.

    entry  = the prior-bar breakout-stop level (where the stop fills)
    height = the impulse-leg height the detector measured
    stop   = the pullback's far side (the detector's structural stop)
    target = the measured move (the impulse height) from the entry
    """
    by_ts = {b.t: b for b in bars}
    out: list[TradeSetup] = []
    for sig in detect_pullbacks(bars, PullbackConfig(), "5m"):
        imp_start = by_ts.get(sig.impulse_start_ts)
        imp_top = by_ts.get(sig.impulse_top_ts)
        if imp_start is None or imp_top is None:
            continue
        if sig.direction == "long":
            height = imp_top.h - imp_start.l
            target = sig.entry_price + height
        else:
            height = imp_start.h - imp_top.l
            target = sig.entry_price - height
        if height <= 0:
            continue
...
def simulate_baseline(sig: TradeSetup, bars: list[Bar],
...
def simulate_scaled(sig: TradeSetup, bars: list[Bar],
                    cost_mult: float = 1.0) -> dict | None:
...
def main() -> int:
    ...
    baseline: list[dict] = []
    n_sig = 0
    for symbol, session_date, bars in sessions:
        for sig in setups_for_session(bars):
            n_sig += 1
            scaled = simulate_scaled(sig, bars)
            base = simulate_baseline(sig, bars)
            if scaled is None:
                continue
    ...
    print(f"  {n_sig} pullback signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "small-pullback breakout-stop (pullback_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "prior-bar breakout-stop level",
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x impulse height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x impulse height "
                    f"beyond the pullback's far side",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = impulse-leg height",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in pullbacks"),
        "longs": summarize(longs, "scaled-in long pullbacks"),
        "shorts": summarize(shorts, "scaled-in short pullbacks"),
    }
    ...
    for mult in (0.5, 1.0, 2.0):
        rs = [s for symbol, sd, bars in sessions
              for sig in setups_for_session(bars)
              if (s := simulate_scaled(sig, bars, cost_mult=mult)) is not None]
    ...
    report_path = OUT_DIR / "pullback_scaled_backtest_report.json"
    ...
    print("\n=== PULLBACK BACKTEST — scaling in behind a wide stop ===")
