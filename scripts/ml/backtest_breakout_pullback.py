#!/usr/bin/env python3
"""Backtest of the Brooks breakout-pullback, traded by SCALING IN
behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the breakout pullback —
the first pullback after a fresh breakout. The detector
(scripts/ml/breakout_pullback_detector.py) is unchanged; the simulate
...
For the breakout pullback the measured move is the breakout leg; the
wide stop is widened beyond the pullback extreme. Long and short
breakouts use identical mirrored rules — no bias.
...
Usage:
    python3 scripts/ml/backtest_breakout_pullback.py
"""
...
from breakout_pullback_detector import detect_breakout_pullbacks  # noqa: E402
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's breakout-pullback signals into normalised
    TradeSetups. The detector already supplies the re-entry, the
    structural stop, the measured-move target and the leg height."""
    out: list[TradeSetup] = []
    for sig in detect_breakout_pullbacks(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} breakout-pullback signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "breakout pullback (breakout_pullback_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "stop 1 tick beyond the prior bar (with-trend re-entry)",
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x breakout-leg height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x breakout-leg height "
                    f"beyond the pullback extreme",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the breakout leg",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in breakout pullback"),
        "longs": summarize(longs, "scaled-in long breakout pullback"),
        "shorts": summarize(shorts, "scaled-in short breakout pullback"),
    }
    ...
    report_path = OUT_DIR / "breakout_pullback_scaled_backtest_report.json"
    ...
    print("\n=== BREAKOUT-PULLBACK BACKTEST — scaling in behind a wide stop ===")
