#!/usr/bin/env python3
"""Backtest of the Brooks final-flag reversal, traded by SCALING IN
behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the final flag — the
failed flag breakout that ends a trend. The detector
(scripts/ml/final_flag_detector.py) is unchanged; the simulate logic is
a deliberate per-setup copy so each backtest stays self-contained and
...
For the final flag the measured move is the final push beyond the
flag; the wide stop is widened beyond that push. A bull-trend final
flag shorts, a bear-trend one goes long — no bias.
...
Usage:
    python3 scripts/ml/backtest_final_flag.py
"""
...
from final_flag_detector import detect_final_flags  # noqa: E402
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's final-flag signals into normalised
    ...
    out: list[TradeSetup] = []
    for sig in detect_final_flags(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} final-flag signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "final-flag reversal (final_flag_detector)",
            ...
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x move height "
                    f"beyond the final push",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the final push beyond the flag",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in final flag"),
        "longs": summarize(longs, "scaled-in long final flag"),
        "shorts": summarize(shorts, "scaled-in short final flag"),
    }
    ...
    report_path = OUT_DIR / "final_flag_scaled_backtest_report.json"
    ...
    print("\n=== FINAL-FLAG BACKTEST — scaling in behind a wide stop ===")
