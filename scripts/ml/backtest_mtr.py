#!/usr/bin/env python3
"""Backtest of the Brooks major trend reversal (MTR), traded by SCALING
IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the major trend reversal.
The detector (scripts/ml/mtr_detector.py) is unchanged; the simulate
...
For the MTR the measured move is the distance from the old extreme to
the structure-break; the wide stop is widened beyond the failed test.
A bull-trend MTR shorts, a bear-trend MTR goes long — no bias.
...
Usage:
    python3 scripts/ml/backtest_mtr.py
"""
...
from mtr_detector import detect_mtrs  # noqa: E402
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's major-trend-reversal signals into normalised
    ...
    out: list[TradeSetup] = []
    for sig in detect_mtrs(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} MTR signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "major trend reversal (mtr_detector)",
            ...
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x move height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x move height "
                    f"beyond the failed test",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = old extreme to the structure break",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in MTR"),
        "longs": summarize(longs, "scaled-in long MTR (bear-trend)"),
        "shorts": summarize(shorts, "scaled-in short MTR (bull-trend)"),
    }
    ...
    report_path = OUT_DIR / "mtr_scaled_backtest_report.json"
    ...
    print("\n=== MTR BACKTEST — scaling in behind a wide stop ===")
