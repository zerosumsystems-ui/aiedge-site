#!/usr/bin/env python3
"""Backtest of the Brooks High-2 / Low-2 pullback, traded by SCALING IN
behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the High-2 / Low-2 — the
two-legged with-trend pullback Brooks calls the most reliable trend
entry. The detector (scripts/ml/h2l2_detector.py) is unchanged; the
simulate logic is a deliberate per-setup copy so each backtest stays
...
For the H2/L2 the measured move is the impulse leg into the swing the
pullback hangs from; the wide stop is widened beyond the pullback's far
extreme. Longs (High 2) and shorts (Low 2) use identical mirrored rules
— no bias.
...
Usage:
    python3 scripts/ml/backtest_h2l2.py
"""
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's High-2 / Low-2 signals into normalised
    TradeSetups. The detector already supplies the breakout entry, the
    structural stop, the measured-move target and the impulse height."""
    out: list[TradeSetup] = []
    for sig in detect_h2l2(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} H2/L2 signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "High-2 / Low-2 pullback (h2l2_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "stop 1 tick beyond the H2/L2 signal bar",
            ...
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x impulse height "
                    f"beyond the pullback's far extreme",
        ...
        "all": summarize(trades, "scaled-in H2/L2"),
        "longs": summarize(longs, "scaled-in High 2 (long)"),
        "shorts": summarize(shorts, "scaled-in Low 2 (short)"),
    }
    ...
    report_path = OUT_DIR / "h2l2_scaled_backtest_report.json"
    ...
    print("\n=== H2/L2 BACKTEST — scaling in behind a wide stop ===")
