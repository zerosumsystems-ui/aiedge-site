#!/usr/bin/env python3
"""Backtest of the Brooks trend-channel-line overshoot reversal, traded
by SCALING IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the trend-channel-line
overshoot. The detector (scripts/ml/channel_overshoot_detector.py) is
...
For the channel overshoot the measured move is the move from the
channel into the reversal; the wide stop is widened beyond the
overshoot push. A bull channel shorts, a bear channel goes long — no bias.
...
Usage:
    python3 scripts/ml/backtest_channel_overshoot.py
"""
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's channel-overshoot signals into normalised
    ...
    out: list[TradeSetup] = []
    for sig in detect_channel_overshoots(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} channel-overshoot signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "trend-channel-line overshoot (channel_overshoot_detector)",
            ...
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x move height "
                    f"beyond the overshoot push",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the move into the reversal",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in channel overshoot"),
        "longs": summarize(longs, "scaled-in long channel overshoot"),
        "shorts": summarize(shorts, "scaled-in short channel overshoot"),
    }
    ...
    report_path = OUT_DIR / "channel_overshoot_scaled_backtest_report.json"
    ...
    print("\n=== CHANNEL-OVERSHOOT BACKTEST — scaling in behind a wide stop ===")
