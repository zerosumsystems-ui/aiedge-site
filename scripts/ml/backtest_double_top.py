#!/usr/bin/env python3
"""Backtest of the Brooks double-top / double-bottom reversal, traded by
SCALING IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the double top / double
bottom — two tests of the same extreme that fail and reverse. The
detector (scripts/ml/double_top_detector.py) is unchanged; the simulate
logic is a deliberate per-setup copy so each backtest stays
...
For the double top / bottom the measured move is the pattern's height
(the tops to the trough between them); the wide stop is widened beyond
the higher top. A double top shorts, a double bottom goes long —
identical mirrored rules, no bias.
...
Usage:
    python3 scripts/ml/backtest_double_top.py
"""
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's double-top / bottom signals into normalised
    TradeSetups. The detector already supplies the reversal entry, the
    structural stop, the measured-move target and the pattern height."""
    out: list[TradeSetup] = []
    for sig in detect_double_tops(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} double-top/bottom signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "double-top / double-bottom reversal (double_top_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "stop 1 tick beyond the reversal trigger bar",
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x pattern height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x pattern height "
                    f"beyond the higher top",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the pattern height",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in double-top/bottom"),
        "longs": summarize(longs, "scaled-in double bottom (long)"),
        "shorts": summarize(shorts, "scaled-in double top (short)"),
    }
    ...
    report_path = OUT_DIR / "double_top_scaled_backtest_report.json"
    ...
    print("\n=== DOUBLE-TOP/BOTTOM BACKTEST — scaling in behind a wide stop ===")
