#!/usr/bin/env python3
"""Backtest of the Brooks climax / exhaustion reversal, traded by SCALING
IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the climax — the
oversized exhaustion bar that is faded. The detector
(scripts/ml/climax_detector.py) is unchanged; the simulate logic is a
deliberate per-setup copy so each backtest stays self-contained and
...
For the climax the measured move is the climax bar's own range; the
wide stop is widened beyond the climax bar. A buying climax shorts, a
selling climax goes long — no bias.
...
Usage:
    python3 scripts/ml/backtest_climax.py
"""
...
from climax_detector import detect_climaxes  # noqa: E402
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's climax signals into normalised
    TradeSetups. The detector already supplies the reversal entry, the
    structural stop, the measured-move target and the move height."""
    out: list[TradeSetup] = []
    for sig in detect_climaxes(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} climax signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "climax / exhaustion reversal (climax_detector)",
            ...
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x climax-bar range",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x climax-bar range "
                    f"beyond the climax bar",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the climax bar range",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in climax"),
        "longs": summarize(longs, "scaled-in long climax (selling climax)"),
        "shorts": summarize(shorts, "scaled-in short climax (buying climax)"),
    }
    ...
    report_path = OUT_DIR / "climax_scaled_backtest_report.json"
    ...
    print("\n=== CLIMAX BACKTEST — scaling in behind a wide stop ===")
