#!/usr/bin/env python3
"""Backtest of the Brooks wedge / three-push reversal, traded by
SCALING IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the wedge — the
three-push reversal pattern. The detector (scripts/ml/wedge_detector.py)
is unchanged; the simulate logic is a deliberate per-setup copy so each
...
For the wedge the measured move is the wedge's height (push 3 to the
wedge's far side); the wide stop is widened beyond push 3. A bull wedge
shorts, a bear wedge goes long — identical mirrored rules, no bias.
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's wedge reversal signals into normalised
    TradeSetups. The detector already supplies the reversal entry, the
    structural stop, the measured-move target and the wedge height."""
    out: list[TradeSetup] = []
    for sig in detect_wedges(bars5, "5m"):
...
def main() -> int:
    ...
    print(f"  {n_sig} wedge signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "wedge / three-push reversal (wedge_detector)",
            ...
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x wedge height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x wedge height "
                    f"beyond push 3",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the wedge height",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in wedge"),
        "longs": summarize(longs, "scaled-in long wedge (bear wedge)"),
        "shorts": summarize(shorts, "scaled-in short wedge (bull wedge)"),
    }
    ...
    report_path = OUT_DIR / "wedge_scaled_backtest_report.json"
    ...
    print("\n=== WEDGE BACKTEST — scaling in behind a wide stop ===")
