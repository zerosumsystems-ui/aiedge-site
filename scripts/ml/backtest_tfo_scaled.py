#!/usr/bin/env python3
"""Backtest of the trend-from-the-open (TFO) setup, traded by SCALING IN
behind a single WIDE protective stop.

This applies the exact Brooks scaling-in trade management proven out on
the spike setup (scripts/ml/backtest_spike.py) to a second setup. The
simulate logic is a deliberate per-setup copy — each backtest stays
self-contained and independently tunable.

The detector (scripts/live/tfo_detector.py) is unchanged: a TFO signal
is the session extreme inside the first four 5-minute bars, followed by
three consecutive in-direction closes (>= 2 of them Brooks-strong). Only
the TRADE MANAGEMENT is the scaling-in method.
...
For TFO the measured move is the impulse from the pivot (the session
extreme) to the signal-bar close; the wide stop is widened beyond that
pivot. Longs and shorts use identical mirrored rules — no bias.
...
Usage:
    python3 scripts/ml/backtest_tfo_scaled.py
"""
...
SCALE_STEP_FRAC = 0.30       # tranche spacing, as a fraction of the
                             # pivot-to-entry impulse height
STOP_WIDEN_FRAC = 1.50       # the wide stop sits this fraction of the
                             # impulse height beyond the structural stop
BREAKEVEN_ARM_FRAC = 0.45    # once favourable excursion clears the
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's TFO signals into normalised TradeSetups.

    entry  = close of the 3rd confirming bar
    height = the impulse from the pivot (session extreme) to that close
    stop   = one tick beyond the pivot extreme
    target = the measured move (the impulse height) from the entry
    """
    out: list[TradeSetup] = []
    for sig in detect_tfo(bars5):
        fire = bars5[sig.fired_bar_index]
        pivot = bars5[sig.pivot_index]
        if sig.direction == "long":
            pivot_extreme = pivot.l
            height = fire.c - pivot_extreme
            stop = pivot_extreme - TICK
            target = fire.c + height
        else:
            pivot_extreme = pivot.h
            height = pivot_extreme - fire.c
            stop = pivot_extreme + TICK
            target = fire.c - height
        if height <= 0:
            continue
        out.append(TradeSetup(
            ...
            entry_index=sig.fired_bar_index,
            entry_ts=fire.t,
            entry_price=fire.c,
...
def main() -> int:
    ...
    print(f"  {n_sig} TFO signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "trend-from-the-open (tfo_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "close of 3rd confirming bar",
            ...
            "target": "measured move = pivot-to-entry impulse height",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in TFO"),
        "longs": summarize(longs, "scaled-in long TFO"),
        "shorts": summarize(shorts, "scaled-in short TFO"),
    }
    ...
    report_path = OUT_DIR / "tfo_scaled_backtest_report.json"
    ...
    print("\n=== TFO BACKTEST — scaling in behind a wide stop ===")
