#!/usr/bin/env python3
"""Backtest of the Brooks inside-bar (ii / iii) breakout, traded by
SCALING IN behind a single WIDE protective stop.

This applies the Brooks scaling-in trade management proven out on the
spike setup (scripts/ml/backtest_spike.py) to the inside-bar breakout —
the two-or-more consecutive inside bars Brooks calls a breakout-mode
pattern. The detector (scripts/ml/ii_detector.py) is unchanged; the
...
For the ii / iii the measured move is the leg into the inside-bar
pattern; the wide stop is widened beyond the far side of the pattern.
...
Usage:
    python3 scripts/ml/backtest_ii.py
"""
...
def setups_for_session(bars5: list[Bar5m]) -> list[TradeSetup]:
    """Turn this session's ii / iii breakout signals into normalised
...
def main() -> int:
    ...
    print(f"  {n_sig} ii/iii signals detected, {len(trades)} simulated")
    ...
    report = {
        "config": {
            "setup": "inside-bar ii/iii breakout (ii_detector)",
            "data": "public/analogs/*/session.json (5-minute RTH)",
            "entry": "stop 1 tick beyond the last inside bar",
            "scale_in": f"{N_TRANCHES - 1} pullback adds, "
                        f"step {SCALE_STEP_FRAC} x leg height",
            "stop": f"single wide stop, {STOP_WIDEN_FRAC} x leg height "
                    f"beyond the far side of the pattern",
            "breakeven_arm_frac": BREAKEVEN_ARM_FRAC,
            "target": "measured move = the leg into the pattern",
            "commission_per_share": COMMISSION_PER_SHARE,
            "entry_slippage_bps": ENTRY_SLIPPAGE_BPS,
            "stop_slippage_bps": STOP_SLIPPAGE_BPS,
        },
        "baseline_all": summarize(baseline, "baseline (single entry, tight stop)"),
        "all": summarize(trades, "scaled-in ii/iii"),
        "longs": summarize(longs, "scaled-in long ii/iii"),
        "shorts": summarize(shorts, "scaled-in short ii/iii"),
    }
    ...
    report_path = OUT_DIR / "ii_scaled_backtest_report.json"
    ...
    print("\n=== ii/iii BACKTEST — scaling in behind a wide stop ===")
